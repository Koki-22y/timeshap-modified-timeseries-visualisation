...
from timeshap.src.timeshap.explainer import local_report, local_event
from timeshap.src.timeshap.explainer.pruning import local_pruning
...
data['id'] = 1
data['all_id'] = 'combined_1'

# Feature setup
raw_model_features = ['combined']
data = data[['timestamp', 'combined', 'id', 'all_id']]
...
def build_model(input_size):
    class ExplainedRNN(nn.Module):
        def __init__(self, input_size: int, cfg: dict):
            ...
            self.rnn = nn.RNN(input_size, self.hidden_dim, batch_first=True, num_layers=2)
            self.linear = nn.Linear(self.hidden_dim, input_size)
        def forward(self, x, hidden_state=None):
            if hidden_state is not None:
                output, hn = self.rnn(x, hidden_state)
            else:
...
# ★ここを修正: NumPy配列をPyTorchテンソルに変換してからモデルに渡し、NumPy配列に戻す
f_hs = lambda x_np: model_wrapped.model(torch.from_numpy(x_np).float())[0].detach().cpu().numpy()
...
report.save('local_report_spring_RNN.html')
...
final_chart.save('dotplot_with_observed_RNN.html')
